...
public class _prRatePerSqFtHistoryGraph extends _prRatePerSqFtHistoryGraphPage {
	...
	public _prRatePerSqFtHistoryGraph() {
		...
		_utils.clickJs($rateByHistoryLink);
		_utils.waitForElementInVisibleByLocator(loader);
	...
	/* Rate per Square Feet History Graph */
	
	public void rateHistory() {
		dateRange3Months();
		getRandomUnits(8);
		unitsValidation(10);
...
//		unitsValidation(10);
	...
	void unitsValidation(int noOfValues) {
	...
	void thisMarket(String marketValueUi, String unitName, String dateValue) {
		...
		String marketValueDb = _databaseUtils.getStringValue(_testData.queryIns.thisMarketUnitRateHistory(unitName, dateValue));
...
//			if(blueMarketDetails.isEmpty()==true) {
	...
	/* Random mouse movement within the specified value */
	...
	void mouseAction() {
		...
		act.moveToElement(_base.driver.findElement(By.cssSelector("g.lines")), randomXPoint, 0);
		act.build().perform();
	}
