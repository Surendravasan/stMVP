...
public class _allStores_copybeforeUK extends _allStoresPage {
	
	ExtentTest test = _base.test;
	ExtentTest node;
	HashMap<String, String> gridHeaderValue;
	
	public _allStores_copybeforeUK() {
		super();
	...
	public void allStoresGrid() {
		...
		if($noCompetitorText.size()!=1) {
			int noOfStoresToCheck = ($storeList.size()>3) ? 3 : $storeList.size(); 
